...
MeEncoderOnBoard::MeEncoderOnBoard(uint8_t slot)
{
  ...
  _Port_PWM_A = encoder_Port[slot].port_PWM_A;
  _Port_PWM_B = encoder_Port[slot].port_PWM_B;
  _Port_ERR_STATE = encoder_Port[slot].port_Err_State;
...
void MeEncoderOnBoard::setMotorPwm(int16_t pwm)
{
  ...
  */
  ...
  if(pwm < 0)
  {
	//Serial.print("set pwm forward");
	//Serial.print("set pwm pin:");
	//Serial.print(MeEncoderOnBoard::_Port_PWM_B);
	analogWrite(MeEncoderOnBoard::_Port_PWM_B, abs(Temp));	
	digitalWrite(MeEncoderOnBoard::_Port_PWM_A, HIGH);//??LOW);
  }
  else
  { 
	//Serial.print("set pwm backward");
	digitalWrite(MeEncoderOnBoard::_Port_PWM_B, HIGH);//LOW);
	//Serial.print("set pwm pin:");
	//Serial.print(MeEncoderOnBoard::_Port_PWM_A);
	analogWrite(MeEncoderOnBoard::_Port_PWM_A, abs(Temp));
  }
}
